...
Function Faucet(){
  while(true){
    ...
    if(preg_match("/Just a moment/",$r)){print k." cloudflare".n;unlink("cookie.".name);die;
    }
    ...
    if($capt){
        $cap = Turnstile($r,host."faucet");
        if(!$cap){continue;}
        $data ="csrf_token_name=$csrf&token=$tok&captcha=$capt&cf-turnstile-response=$cap&human_move=1";
        $r = Bypass(host."faucet/verify",h(),$data);
        $ss = Ambil($r," title: '","',",1);
        $ubl = Ambil($r,'<h3>','</h3>',3);
        if($ss){
